...
class TaskHandler:
    ...
    def handle_task(self, envelope: dict):
        ...
        task = None
        try:
            task = self._parse_task_from_request(envelope)
            ...
            # Fängt erwartete Fehler (Parsing, I/O) und wirft sie zur spezifischen
            # HTTP-Fehlerbehandlung im Web-Layer weiter.
            raise e
        except Exception as e:
            # Fängt unerwartete Fehler und versucht, den Task-Status zu aktualisieren,
            # bevor der Fehler weitergeworfen wird.
    ...
    def _parse_task_from_request(self, envelope: dict) -> task_pb2.Task:
        """Extrahiert, dekodiert und parst den Task aus der Pub/Sub-Nachricht."""
